...
public class Board extends JPanel implements MouseListener  , ActionListener
{
	...
	static public boolean gameInProgress;
	...
	Board()
	{
		...
		winMessage.setFont(new Font("Sefir", Font.BOLD, 20));
		//winMessage.setBounds(Checkers.FRAME_SIZE/2, Checkers.FRAME_SIZE/2, cellSize, cShift);
	...
	public void mousePressed (MouseEvent evt) 
	{
		...
		{
			message.setText("There is no Game Happening, please click new game.");
		}

		//makes sure the click is on the board.
		else if (clickX> leftBound && clickX<rightBound && clickY>topBound && clickY<bottomBound)
		{
			row= (clickX-rShift)/(cellSize);
			col=(clickY-cShift)/(cellSize);
		
			if(!jumpFlag) //checks to see if the should keep jumping
			{	
			...
			else if(CheckersData.canJump(currentPlayer, selectedRow, selectedCol, row,col))
			{
					doClickSquare(row, col);
			}
			else 
	...
	void doClickSquare(int row, int col) 
	{
		...
		if (selectedRow<0) 
		{
			...
			{	
				selectedRow=row;
				selectedCol=col;
				message.setText("you have selected your Red king at "+ selectedRow+ " , "+ selectedCol);
			...
			else if (currentPlayer==CheckersData.BLACK && board.pieceAt(row, col)==CheckersData.BLACKKING)
			{
				selectedRow=row;
				selectedCol=col;
				message.setText("you have selected your Black king at "+ selectedRow+ " , "+ selectedCol);
		...
		else // if there is a piece selected this checks if the click was a valid move or jump
		{
			// this checks if the click was a move
			if (CheckersData.canMove(currentPlayer, selectedRow, selectedCol, row, col))
			{
				System.out.println(" can move");
				CheckersData.doMove(selectedRow, selectedCol, row, col);
				
				if (!CheckersData.checkKing(row, col))
				{
			...
			}// end check if it can move there
			
			// this checks if the click is a possible jump
			else if (CheckersData.canJump(currentPlayer, selectedRow, selectedCol, row, col))
			{
				
				CheckersData.doJump(selectedRow, selectedCol, row, col, (selectedRow+row)/2, (selectedCol+col)/2);
				...
				selectedCol=col;
				jumpFlag=true;
				CheckersData.checkKing(selectedRow, selectedCol);
				
					if (jumpFlag==true)
					{
						
							if (CheckersData.checkJump(currentPlayer, selectedRow, selectedCol)>0)
							{
							...
							else  // if they can't make another jump
							{
								...
								{
									currentPlayer= CheckersData.RED;
									message.setText("Red player: it it now your turn, please select a piece");
									selectedRow=-1;
								}
